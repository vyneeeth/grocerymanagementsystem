...
<body background= "../111.png">
    ...
    <div class="pdc">
        <form action='user_edit.php' method='post' onsubmit='return edit1()' style="display: flex; flex-direction: column; font-family: Poppins; font-weight: 600;">
            ...
            <input type="submit" style="width: 150px; height: 20px; margin-top: 0px; border: 1px solid rgba(149, 231, 187, 1); background: rgb(219, 244, 230);">
        </form>
        <p id='hid'></p>
        <script>
            function edit1(){
            <?php
                ...
                $adr = $_POST['adr'];
                $rolll = $_POST['rolll'];
                $sql = "UPDATE user SET pwd=?, name=?, email=?, mobile=?, addressid=?, role=? WHERE userid=?";
                $stmt= $conn->prepare($sql);
                $stmt->bind_param("sssiisi", $pwd, $nam, $email, $phn, $adr, $rolll, $id);
                $stmt->execute();
            ?>
            }
